...
############################  ListBackupSetFiles.sh  ###########################
# You can use this shell script to list all the files within a backup set from #
# the command line. Just customize the "User Defined Section" below with       #
# your values for the required action.                                         #
################################################################################
...
MAIN_CLASS=ListBackupSetFiles

echo "Using APP_HOME     : ${APP_HOME}"
echo "Using SETTING_HOME : ${SETTING_HOME}"
echo "Using BACKUP_SET   : ${BACKUP_SET}"
echo "Using Result File  : ${APP_HOME}/dump.xml"

# API Arguments: ListBackupSetFiles [APP_HOME] [BACKUP_SET] [SETTING_HOME]
...
"${JAVA_EXE}" $JAVA_OPTS $JNI_PATH -cp $CLASSPATH $MAIN_CLASS "${APP_HOME}" "${BACKUP_SET}" "${SETTING_HOME}" > dump.xml
