...
if [ ! -x "$APP_HOME/jre32" ];
then
    echo "'$APP_HOME/jre32' does not exist!"
    if [ ! -n "$JAVA_HOME" ]; then
        echo "Please set JAVA_HOME!"
        exit 0
    else
        ln -sf "$JAVA_HOME" "$APP_HOME/jre32"
        echo "Created JAVA_HOME symbolic link at '$APP_HOME/jre32'"
    fi
fi

if [ ! -x "$APP_HOME/jre32" ];
then
    echo "Please create symbolic link for '$JAVA_HOME' to '$APP_HOME/jre32'"
    exit 0
fi

JAVA_HOME="$APP_HOME/jre32"
...
if [ "Darwin" = `uname` ]; then
    ...
    if [ ! -L "$JAVA_HOME/bin/bJW" ]
    then
        rm -f "$JAVA_HOME/bin/bJW"
        echo "Create Backup Manager JVM, Path: $JAVA_HOME/bin/bJW"
        ln -sf "$JAVA_HOME/bin/java" "$JAVA_HOME/bin/bJW"
        chmod 755 "$JAVA_HOME/bin/bJW"
    fi
fi

# Verify the JAVA_EXE (original file) whether it can be executed or not.
if [ ! -x "${JAVA_EXE}" ]
then
    if [ $NAME_IS_JAVA -eq 0 ]
    then
        # If the symlink is not found, using the Java Executable instead.
